...
import CargoMap from '@/components/CargoMap.vue';
...
export default {
  ...
  methods: {
    ...
    getResult() {
      ...
      this.$http.post("/result/cargo", { uuid: this.uuid })
        .then((res) => {
          ...
          let cargoMap = this.$refs.cargoMap;
          cargoMap.plot(res["data"]);
          this.pollingCount--;
          this.stage = "solving";
    ...
    // 随机数据生成
    random() {
      const carList = this.randomCar(3);
      const storageList = this.randomStorage(2);
      const requestList = this.randomRequest(60, storageList);
      let data = {
        "carList": carList,
