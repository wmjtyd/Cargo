...
import OptaplannerGoogleMap from '@/components/OptaplannerGoogleMap.vue';
...
export default {
  components: {
    OptaplannerGoogleMap, MyHeader, Status
  },
  data() {
    return {
      id: 0,
    ...
    getResult() {
      ...
      this.$http.post("/result/routing", { uuid: this.uuid })
        .then((res) => {
          ...
          let optaplannerGoogleMap = this.$refs.optaplannerGoogleMap;
          optaplannerGoogleMap.plot(res["data"]);
          this.pollingCount--;
          this.stage = "solving";
